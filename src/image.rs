...
pub trait ImageOperations: GraphicsRenderer {
    ...
    fn draw_image_rgb(&mut self, x: u16, y: u16, img: &impl GenericImageView<Pixel = Rgb<u8>>);
    /// Brief - Method for drawing an image with an alpha channel, slower
    fn draw_image_rgba(&mut self, x: u16, y: u16, img: &impl GenericImageView<Pixel = Rgba<u8>>);
}
...
impl<T: GraphicsRenderer> ImageOperations for T {
    fn draw_image_rgb(&mut self, x: u16, y: u16, img: &impl GenericImageView<Pixel = Rgb<u8>>) {
        for (x, y, p) in img.pixels() {
            let c = p.to_rgb();
            self.set_pixel(T::Color::new(c[0], c[1], c[2]), x as u16, y as u16);
        }
    }
    fn draw_image_rgba(&mut self, x: u16, y: u16, img: &impl GenericImageView<Pixel = Rgba<u8>>) {
        for (x, y, p) in img.pixels() {
            let c = p.to_rgba();
            self.blend_pixel(T::Color::new(c[0], c[1], c[2]), x as u16, y as u16, c[3]);
        }
    }
}
