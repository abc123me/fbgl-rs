...
impl<T: GraphicsRenderer, const N: usize> MultiDisplayHorizontalRenderer<T, N> {
	...
	fn set_pixels_impl(&mut self, pixels: &Vec<T::Color>, x1: u32, y1: u32, x2: u32, y2: u32) {
		let w = self.get_width();
		for y in y1..y2 {
			let p = y * w;
			for x in x1..x2 {
	...
	for MultiDisplayHorizontalRenderer<T, N>
{
	...
	fn set_pixels(&mut self, pixels: &Vec<Self::Color>, x1: u32, y1: u32, x2: u32, y2: u32) {
		...
		for i in 0..(self.renderers.len() as u32) {
			let rx1 = i * self.base_w;
			let rx2 = rx1 + self.base_w;
			if x1 < rx2 {
				self.set_pixels_impl(pixels, x1.max(rx1), y1, x2.min(rx2), y2);
			}
		}
	}
}

impl<T: GraphicsRenderer + GraphicsOperations, const N: usize> GraphicsOperations
	for MultiDisplayHorizontalRenderer<T, N>
{
	fn clear(&mut self, col: T::Color) {
		for rend in &mut self.renderers {
			rend.clear(col);
		}
	}
}
